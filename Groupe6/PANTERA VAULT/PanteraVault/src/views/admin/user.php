...
try {
    ...
    var_dump('conexion ok');
} catch (PDOException $e) {
    echo 'Connexion échouée : ' . $e->getMessage();
}
//var_dump($dbh);
Function recherche($mescouilles) {
$sql = 	'SELECT   *   FROM  Product 

	INNER JOIN option ON option_idOption = idOption;

	WHERE nomOption LIKE "%'.$mescouilles.'%"';
}
...
?>
    <div class="container col-md-6 table-responsive">
        <table class="justify-content-center d-flex table-bordered">
            ...
            foreach ($result as $user){
                echo '<tr>';
                echo '<td class="table-secondary">'.$user ['nomUser'].'</td>';
                echo '<td class="table-secondary">'.$user ['prenomUser'].'</td>';
                echo '<td class="table-secondary">'.$user ['ageUser'].'</td>';
                echo '<td class="table-secondary">'.$user ['adresseFactureUser'].'</td>';
                echo '<td class="table-secondary">'.$user ['adresseLivraisonUser'].'</td>';
                echo '<td class="table-secondary">'.$user ['dateInscriptionUser'].'</td>';
                echo '<td class="table-secondary">'.$user ['emailUser'].'</td>';
                echo '<td class="table-secondary">'.$user ['mdpUser'].'</td>';
                echo '<td class="table-secondary">'.$user ['roleUser'].'</td>';
                echo '<td class="table-secondary">'.$user ['statutUser'].'</td>';
                echo '<td class="table-warning"><a href="user-detail.php?id='.$user['idUser'].'">Editer</a>/<a href="delete-user.php?id='.$user['idUser'].'">supprimer</a></td>';
                echo '</tr>';
            }
            ?>
        </table>
